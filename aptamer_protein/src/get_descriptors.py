...
def extractPAAC(seq, props=["Hydrophobicity", "Hydrophilicity", "SideChainMass"], 
    ...
    default_properties = {
        "Hydrophobicity": [0.62, -2.53, -0.78, -0.9, 0.29, -0.74, -0.85, 0.48, -0.4, 1.38, 1.06, -1.5, 0.64, 1.19, 0.12, -0.18, -0.05, 0.81, 0.26, 1.08],
    ...
    if customprops is not None:
        for index, row in customprops.iterrows():
            all_properties[row['AccNo']] = [row['A'], row['R'], row['N'], row['D'], row['C'], row['E'], row['Q'], row['G'], row['H'], row['I'], row['L'], row['K'], row['M'], row['F'], row['P'], row['S'], row['T'], row['W'], row['Y'], row['V']]
