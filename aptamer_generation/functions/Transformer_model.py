...
class SourceEmbeddings(nn.Module):
    def __init__(self, input_dim: int, d_model: int):
        """
        input_dim: dimensionality of your raw source embeddings (2048)
        d_model: transformer hidden dimension (512)
        """
        super().__init__()
        self.d_model = d_model
...
# Creating a positional encoding matrix of shape (seq_len, d_model) filled with zeros
        ...
        pos = torch.arange(0, seq_len, dtype = torch.float).unsqueeze(1) # Transforming 'position' into a 2D tensor['seq_len, 1']
    ...
    def forward(self, x):
        ...
        #x.shape[1] — текущая длина последовательности
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def __init__(self, d_model: int, h: int, dropout: float) -> None: # h = number of heads
        ...
        self.d_model = d_model
        self.h = h
        self.attention_scores = None  # Add this to store attention
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, encoder_output, src_mask, tgt_mask):
        ...
        ca_out = self.cross_attention_block(x, encoder_output, encoder_output, src_mask)
...
class Transformer(nn.Module):
    def __init__(self, encoder: Encoder, decoder: Decoder, src_embed: nn.Module,
                 src_pos: PositionalEncoding, tgt_embed: InputEmbeddings, tgt_pos: PositionalEncoding,
    ...
    def forward(self, encoder_input, encoder_mask, decoder_input, decoder_mask):
        ...
        decoder_output = self.decode(memory, encoder_mask, decoder_input, decoder_mask)
        return self.project(decoder_output)




def build_transformer(tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, d_model: int = 512, N: int = 12, h: int = 16, dropout: float = 0.1, d_ff: int = 2048, input_dim: int = 2048):
    tgt_embed = InputEmbeddings(d_model, tgt_vocab_size)
    src_embed = SourceEmbeddings(input_dim, d_model=d_model)
    ...
    transformer = Transformer(encoder, decoder, src_embed, src_pos,
                            tgt_embed, tgt_pos, projection_layer)
